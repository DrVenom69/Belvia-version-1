  ...
  return (
    <section id="imported-section" className="py-16 bg-bg-base relative">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative text-left">
        ...
        {/* Categories Sorters */}
        <div className="flex border-b border-gray-805 mb-8 overflow-x-auto">
        ...
        {/* Info Banner */}
        <div className="p-4 rounded-xl bg-accent-secondary/10 border border-accent-secondary/20 flex items-start space-x-3 text-xs text-gray-400 mb-8 max-w-3xl">
          <Info className="w-5 h-5 text-accent shrink-0 mt-0.5" />
          <div className="space-y-1">
            <span className="text-white font-bold block">Pre-Order Deposit System</span>
        ...
        {filteredProducts.length === 0 ? (
          <div className="text-center py-20 bg-bg-base/40 border border-gray-850/60 rounded-2xl max-w-md mx-auto">
            <Plane className="w-12 h-12 text-gray-650 mx-auto mb-3 animate-pulse" />
            <h4 className="text-white font-bold font-display text-base">All Batches Dispatched</h4>
            <p className="text-gray-405 text-xs mt-1.5 leading-relaxed">
            ...
            {filteredProducts.map((p) => {
              ...
              return (
                <div
                  ...
                  key={p.id}
                  className="rounded-2xl bg-bg-surface/75 border border-bg-elevated hover:border-gray-700 hover:shadow-2xl hover:shadow-accent/5 transition duration-300 overflow-hidden flex flex-col justify-between"
                >
                  {/* Thumbnail Banner */}
                  <div className="aspect-[16/10] w-full bg-bg-surface relative">
                    ...
                    {/* Origin Tag */}
                    <div className="absolute top-3 left-3 px-2 py-1 rounded bg-bg-base/90 text-[10px] font-mono font-bold tracking-widest text-[#fff] border border-gray-850 flex items-center space-x-1.5">
                      <Globe className="w-3.5 h-3.5 text-accent" />
                      <span>{p.originalImportCountry?.toUpperCase() || 'IMPORTED'}</span>
                    </div>

                    {/* Pre-order badge */}
                    <span className="absolute top-3 right-3 px-2.5 py-1 text-[9px] font-mono tracking-widest font-black uppercase rounded bg-orange-600 border border-orange-500 text-white animate-pulse">
                      Pre-Order Only
                    </span>

                    {/* Estimated Cargo Arrival Indicator */}
                    <div className="absolute bottom-3 left-3 right-3 px-3 py-1.5 rounded-lg bg-bg-base/85 backdrop-blur-xs border border-gray-805 flex items-center justify-between text-[10px] font-mono text-gray-300">
                      <span className="flex items-center">
                        <Calendar className="w-3.5 h-3.5 text-orange-400 mr-1.5 shrink-0" />
                        ARRIVES:
                      </span>
                      <span className="font-bold text-gray-100 truncate">{p.estimatedArrival}</span>
                    </div>
                  </div>
                  ...
                  <div className="p-5 flex-grow flex flex-col justify-between">
                    <div className="space-y-2">
                      ...
                      <h3 className="font-display font-black text-lg text-white group-hover:text-accent transition leading-snug line-clamp-1">
                        {p.title}
                      </h3>
                      ...
                      <div className="pt-3 pb-1 grid grid-cols-2 gap-2 text-[10px] font-mono text-gray-400 border-t border-gray-850/60">
                        <div className="flex justify-between pr-2 border-r border-gray-850">
                          <span>SCALE:</span>
                          <span className="text-gray-200">{p.dimensions || 'N/A'}</span>
                        </div>
                        <div className="flex justify-between pl-2">
                          <span>MASS WEIGHT:</span>
                          <span className="text-gray-250">{p.weightGrams}g</span>
                        </div>
                      </div>
                    </div>

                    {/* Pricing Sheet Checkout anchor */}
                    <div className="mt-5.5 pt-4.5 border-t border-bg-elevated flex items-center justify-between">
                      <div>
                        {/* Deposit specification calculations */}
                        <div className="flex items-center text-[9px] font-mono text-gray-500 tracking-wider">
                        ...
                        <div className="flex items-end space-x-1 mt-0.5">
                          ...
                          <span className="text-[10px] font-mono text-gray-400 mb-0.5">({p.depositPercentage}% DEPOSIT)</span>
                        </div>
                      </div>
                      ...
                      <div className="flex space-x-2">
                        <button
                          ...
                          className="p-2.5 rounded-xl border border-bg-elevated bg-bg-base hover:border-gray-700 text-gray-400 hover:text-white text-xs cursor-pointer transition shrink-0"
