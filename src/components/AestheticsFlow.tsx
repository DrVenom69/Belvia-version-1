...
export default function AestheticsFlow() {
  ...
  return (
    <div id="additional-aesthetics-wrapper" className="bg-bg-base relative">
      ...
      <section id="recent-prints" className="py-20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center mb-16 max-w-xl mx-auto">
            ...
            <h2 className="font-display text-2xl sm:text-3xl font-extrabold text-white mt-1">Completed Farm Print Projects</h2>
            <p className="text-gray-400 text-xs mt-2 leading-relaxed">Direct macro close-ups of models created inside our additive bays showing true outer wall finishing standards.</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6.5">
            {PORTFOLIO.map((p, idx) => (
              <div key={idx} className="bg-[#070c15] border border-bg-elevated rounded-2xl overflow-hidden text-left flex flex-col group">
                <div className="aspect-[4/3] w-full bg-bg-surface relative overflow-hidden">
                  ...
                  <span className="absolute top-3 left-3 px-2 py-0.5 rounded text-[9px] font-mono bg-bg-base/90 text-accent font-bold border border-bg-elevated">
                    {p.mate}
                  </span>
                </div>
                <div className="p-4.5 space-y-4 flex-1 flex flex-col justify-between">
                  <div>
                    <h3 className="font-display font-bold text-sm text-gray-100 leading-normal line-clamp-1">{p.title}</h3>
                    
                    {/* Splicer diagnostics */}
                    <div className="grid grid-cols-3 gap-2.5 pt-3.5 border-t border-gray-850 text-[10px] font-mono text-gray-400">
                      <div>
                        <span className="block text-[8px] text-gray-500 uppercase">layer ht</span>
                        <span className="font-bold text-gray-200">{p.layer}</span>
                      </div>
                      <div>
                        <span className="block text-[8px] text-gray-500 uppercase">Net Mass</span>
                        <span className="font-bold text-gray-200">{p.weight}</span>
                      </div>
                      <div>
                        <span className="block text-[8px] text-gray-500 uppercase">hours</span>
      ...
      {/* 3. CUSTOMER REVIEWS */}
      <section id="opinions" className="py-20 bg-bg-base/40 border-t border-bg-elevated">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          
          <div className="text-center mb-16 max-w-xl mx-auto">
            <span className="text-xs font-mono font-bold uppercase tracking-widest text-indigo-400">Industrial Testimonials</span>
            <h2 className="font-display text-2xl sm:text-3xl font-extrabold text-white mt-1">Manufacturer Reviews</h2>
            <p className="text-gray-400 text-xs mt-2 leading-relaxed">Trusted by engineers and individual hobbyist designers for FDM tolerances.</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6.5 text-left">
            {REVIEWS.map((rev, idx) => (
              <div key={idx} className="bg-[#070b13]/85 p-6 border border-gray-805 rounded-2xl flex flex-col justify-between space-y-4">
                <p className="text-gray-300 text-xs font-sans italic leading-relaxed">
                  "{rev.t}"
                </p>
                <div className="flex items-center space-x-3.5">
                  ...
                  <div>
                    <h4 className="font-display font-bold text-xs text-white leading-none">{rev.name}</h4>
                    <span className="text-[10px] text-gray-500 font-mono mt-1 block">{rev.r}</span>
                  </div>
                </div>
              </div>
            ))}
      ...
      {/* 4. FAQ ACCORDION */}
      <section id="frequently-asked-fields" className="py-20 border-t border-bg-elevated">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center mb-14 max-w-xl mx-auto">
            ...
            <h2 className="font-display text-2xl sm:text-3xl font-extrabold text-white mt-1">Frequently Asked Slices</h2>
            <p className="text-gray-400 text-xs mt-2 leading-relaxed">Understand our additive calibration protocols and spatial clearances.</p>
          </div>
          ...
          <div className="space-y-3.5 text-left">
            {FAQS.map((faq, idx) => {
              ...
              return (
                <div key={idx} className="bg-[#070b13] border border-gray-805 rounded-xl overflow-hidden transition">
                  <button
                  ...
                  >
                    <span className="font-display font-bold text-sm text-gray-200 hover:text-white transition">{faq.q}</span>
                    <ChevronDown className={`w-4.5 h-4.5 text-gray-400 shrink-0 transition-transform duration-300 ${active ? 'rotate-180 text-accent' : ''}`} />
                  </button>
                  {active && (
                    <div className="px-4.5 pb-4.5 font-sans text-xs text-gray-400 leading-relaxed border-t border-gray-850/40 pt-3">
                      {faq.a}
                    </div>
                  )}
                </div>
      ...
      <section id="cta-catalogue-banner" className="py-14 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="bg-gradient-to-r from-accent-secondary/20 to-accent-secondary/10 border border-accent/20 rounded-3xl p-8 sm:p-12 text-center relative overflow-hidden shadow-2xl">
          ...
          <div className="relative z-10 max-w-xl mx-auto space-y-6">
            <h2 className="font-display text-3xl sm:text-4xl font-extrabold text-white tracking-tight">Need custom physical files modeled?</h2>
            <p className="text-gray-300 text-xs sm:text-sm leading-relaxed">Our print fleets compile 3D assets on premium filaments under 12 hours. Set up custom prints easily with our cloud custom uploader.</p>
      ...
      {/* 6. BRAND FOOTER */}
      <footer id="belvia-brand-footer" className="bg-[#02050b] border-t border-bg-elevated pt-16 pb-8 text-xs font-mono text-gray-500">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 md:grid-cols-4 gap-8 mb-12 text-left">
          
          <div className="space-y-4">
            <div className="flex items-center space-x-2">
              <div className="w-7 h-7 bg-accent-secondary text-white rounded-lg flex items-center justify-center font-display font-bold">B</div>
              <span className="font-display font-black text-base text-white tracking-widest">BELVIA.</span>
            </div>
            <p className="text-gray-600 leading-normal max-w-xs font-sans">Premium e-commerce and manufacturing print fleets executing 3D model structures in standard scale clearances.</p>
          </div>

          <div>
            <h4 className="text-gray-200 text-[10px] uppercase font-bold tracking-widest mb-3.5">Fliaments catalog</h4>
          ...
          <div>
            <h4 className="text-gray-200 text-[10px] uppercase font-bold tracking-widest mb-3.5">Farm diagnostics</h4>
            <ul className="space-y-1.5">
          ...
          <div>
            <h4 className="text-gray-200 text-[10px] uppercase font-bold tracking-widest mb-3.5">corporate procurement</h4>
            <p className="leading-relaxed mb-3 text-gray-600 font-sans">Connect with our additive farm engineers for design revisions and quote files clearance sheet listings.</p>
            <div className="text-accent font-bold hover:underline select-all text-[11px]">procure@belvia3d.com</div>
          </div>

        </div>

        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 border-t border-bg-elevated/60 pt-8 flex flex-col sm:flex-row justify-between items-center text-[10px] gap-4">
          <div className="tracking-wide">© 2026 BELVIA 3D MANUFACTURING LTD. ALL ASSETS SECURED.</div>
          <div className="flex space-x-4">
            <span className="hover:text-gray-300 cursor-pointer">Slicing SLA Policy</span>
            <span className="hover:text-gray-300 cursor-pointer">Security Standards</span>
            <span className="hover:text-gray-300 cursor-pointer">Bambu Studio configs</span>
          </div>
        </div>
      </footer>
